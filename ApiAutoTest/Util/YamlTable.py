...
class YamlTable(object):
	_Title = ['id', 'path', 'method', 'tag', 'host', 'basepath', 'param_method', 'param_required', 'params', 'resps',
	...
	def get_new_paths(self):
		...
		new_path = []
		for new in news:
			new_path.append(self.sheet.cell_value(new,col_path))
		return new_path
	...
	def trans_yaml_one_to_excel(self, file_name):
		...
		file_dir = os.path.join(conf_dir, file_name + ".yaml")
	...
	def _param_format(self, params):
		...
		if len(params) > 0:
			for param in params:
				...
				required.update({param['name'].keys()[0]: param['required']})
				method.update({param['name'].keys()[0]: param['method']})
