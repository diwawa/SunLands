...
pkArena_pkMatch
	[Documentation]    匹配对手 / 点击分享链接，随机匹配
	[Tags]    Run
	${resp}    get/pkArena/pkMatch    ${token}    ${EMPTY}    ${False}
	set suite variable      ${pkLogId}    ${resp['pkLogId']}
	# 随机匹配者信息
	set suite variable     ${userId2}    ${resp['targetUserView']['userId']}
...
pkArena_pkFinal
	[Documentation]    pk结果
	[Tags]    Run
	${result}    PK_随机匹配_结果    ${userId2}    ${pkLogId}    ${Random['isCorrect_1']}    ${Random['currentUserDuration_1']}
