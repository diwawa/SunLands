...
questionTypes_examQuestion
    ...
    ${resp}    get/questionTypes/examQuestion    ${token}    ONLY_WRONG_PRACTICE    0    0    0
    ...    ${chapterId}
    ${len}    Get Length    ${resp}
    ${questionIds}    create list
    : FOR    ${x}    IN RANGE    ${len}
    \    Comment    should be equal as integers    ${resp[${x}]['questionId']}    ${sql_last['question_id_only'][${x}]}
